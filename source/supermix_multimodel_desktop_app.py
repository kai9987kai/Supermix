...
def iter_bundled_model_sources() -> list[Path]:
    found: list[Path] = []
    seen: set[str] = set()
    for root in iter_runtime_roots(None):
        if not root.exists():
        ...
        for child in sorted(root.iterdir()):
            if not child.is_dir():
                continue
            if not child.name.startswith("bundled_models"):
                continue
...
def hydrate_bundled_models(models_dir: Path) -> Path:
    ...
    for bundled in iter_bundled_model_sources():
        # Support .zip, .pth, and .json files
        for pattern in ["*.zip", "*.pth", "*.json"]:
            for source_file in sorted(bundled.glob(pattern)):
                target = models_dir / source_file.name
                try:
                    if target.exists() and target.stat().st_size == source_file.stat().st_size:
                        continue
                    shutil.copy2(source_file, target)
                    logging.info("Hydrated bundled artifact: %s -> %s", source_file.name, target)
                except Exception:
                    logging.exception("Failed to hydrate bundled model artifact %s into %s", source_file, target)
    return models_dir
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--title", default="Supermix Studio X")
