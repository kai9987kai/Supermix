...
$runName = "omni_collective_v46_common_v5_20suite_evo3h"
...
$expandedBenchmarkDetails = Get-ChildItem -Path (Join-Path $repoRoot "output") -Recurse -File -Filter "benchmark_all_models_common_details.jsonl" -ErrorAction SilentlyContinue |
    Where-Object { $_.FullName -match "common_v5_20suite|common_v4_hard|v4_hard|expanded|cachefloor" } |
    Sort-Object LastWriteTime -Descending |
    Select-Object -First 1
...
$pythonArgs = @(
    ...
    ('"{0}"' -f $latestV8Summary),
    "--output_root",
    "output\omni_collective_v46_common_v5_20suite_evo3h_prep",
    "--output_dir",
    "output\omni_collective_v46_common_v5_20suite_evo3h_train",
    "--family_name",
    "omni_collective_v46_common_v5_20suite_evo3h",
    "--artifact_prefix",
    "supermix_omni_collective_v46_common_v5_20suite_evo3h",
    "--seed",
    "2086",
    "--benchmark_limit",
    "320",
...
[pscustomobject]@{
    ...
    target_hours = if ($NoWatchdog) { $null } else { 3 }
    out_log = $outLog
    err_log = $errLog
    ...
    benchmark_suite_version = "common_v5_20suite"
    strategy = "promoted v46 warm-start + 20-suite benchmark replay + weak-suite oversampling + exact-answer cache anchors + Pareto non-regression floor"
    base_branch = [string]$championManifest.champion_family
    base_score = [double]$championManifest.common_benchmark_score
    benchmark_failure_replay_limit = 5200
    suite_best_anchor_limit = 5200
    hard_benchmark_limit = 2200
    new_suites = "qasc, social_iqa, strategyqa, multirc, drop"
    weak_suites_from_baseline = "drop, qasc, strategyqa, multirc, social_iqa"
    old_suite_floor = "preserve promoted 15-suite common_v4 score while lifting the added common_v5 suites"
