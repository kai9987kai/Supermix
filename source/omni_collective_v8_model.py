...
OmniCollectiveNetV8 = OmniCollectiveNetV4


def _prompt_variants_v8(
    prompt: str,
    draft_answer: str = "",
    *,
    ...
    variants = [
        ...
        (
            "Fuse the strongest grounded signal across local specialist domains.\n"
            f"Request: {prompt_text}\n"
            "Check whether coding, math, protein, image, 3D, materials, or general reasoning evidence should dominate, and avoid forcing the wrong specialist voice."
        ),
        (
            "Run a contradiction and omission check.\n"
            f"Request: {prompt_text}\n"
            "Look for hidden conflicts between likely answer candidates, preserve the supported core, and remove weakly grounded extras."
        ),
    ]
    if draft_answer:
        variants.append(
    ...
    if response_confidence < 0.50 or domain_confidence < 0.42:
        ...
        variants.append(
            (
                "Confidence is still low after the first pass.\n"
                f"Request: {prompt_text}\n"
                "Return a verified core answer first, then add one short uncertainty note only if it materially helps."
            )
        )
    if focus == "coding":
        variants.append(
    ...
    if any(token in lowered for token in ("protein", "folding", "amino acid", "residue", "alpha helix", "alpha helices", "beta sheet", "hydrophobic", "chaperone", "contact map")):
        variants.append(
            (
    ...
    if any(token in lowered for token in ("mattergen", "materials", "crystal", "cif", "perovskite", "thermoelectric", "solid electrolyte")):
        variants.append(
            (
                "This request may involve materials or crystal generation.\n"
                f"Request: {prompt_text}\n"
                "Prefer grounded materials terminology, prototype families, and bounded claims about properties rather than pretending to run full physics."
            )
        )
    if any(token in lowered for token in ("3d", "openscad", "stl", "obj", "mesh", "cad", "polyhedron", "phone stand")):
        variants.append(
            (
                "This request may involve 3D or OpenSCAD generation.\n"
                f"Request: {prompt_text}\n"
                "Prefer valid parametric geometry, OpenSCAD primitives, and concise buildable code over decorative filler."
            )
        )
    deduped: List[str] = []
    seen = set()
...
class OmniPredictionV8:
    ...
    reasoning_mode: str = "all_model_multitype_grounded_consensus_deliberation_v8"


class OmniCollectiveEngineV8:
    def __init__(self, *, weights_path: Path, meta_path: Path, device: Optional[torch.device] = None) -> None:
        ...
        self.deliberation_passes = int(self.meta.get("deliberation_passes") or 10)
        self.minimum_passes = int(self.meta.get("minimum_passes") or 5)
        self.grounding_threshold = float(self.meta.get("grounding_threshold") or 0.54)
        self.device = device or torch.device("cuda" if torch.cuda.is_available() else "cpu")

        self.model = OmniCollectiveNetV8(
    ...
    def predict(self, prompt: str, image_path: Optional[str] = None) -> OmniPredictionV8:
        ...
        for index, variant in enumerate(variants, start=1):
            ...
            if "hallucination guard" in lowered:
                weight = 1.10
            elif "contradiction and omission check" in lowered:
                weight = 1.07
            elif "fuse the strongest grounded signal" in lowered:
                weight = 1.06
            ...
            if index >= self.minimum_passes and response_conf >= self.grounding_threshold and domain_conf >= 0.46:
                break
