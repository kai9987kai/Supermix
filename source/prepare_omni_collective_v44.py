...
try:
    from prepare_omni_collective_v43 import latest_common_benchmark_summary_path, load_summary
except ImportError:  # pragma: no cover
    from .prepare_omni_collective_v43 import latest_common_benchmark_summary_path, load_summary


REPO_ROOT = Path(__file__).resolve().parent.parent
DEFAULT_OUTPUT_PATH = REPO_ROOT / "output" / "omni_collective_v44_blueprint.json"
V42_MANIFEST_PATH = Path(__file__).resolve().parent / "v42_benchmax_manifest.json"


RECENT_METHOD_REFERENCES_V44: List[Dict[str, str]] = [
    {
    ...
    },
    {
        ...
        "why": "Distill useful partial traces so v44 can stop earlier on easy problems without losing answer quality.",
    },
    {
    ...
    },
    {
        ...
        "why": "Use Qwen 3.5 for long-context, tool-use, and reasoning traces, especially where v43 is weak against v42_benchmax.",
    },
    {
...
def latest_v43_summary_path(repo_root: Path = REPO_ROOT) -> Path:
    candidates = sorted(
        repo_root.glob("output/supermix_omni_collective_v43_frontier_*/omni_collective_v43_frontier_summary.json"),
        key=lambda item: (item.parent.name, item.stat().st_mtime),
        reverse=True,
    )
    if not candidates:
        raise FileNotFoundError("No omni_collective_v43 frontier summary found under output/.")
    return candidates[0].resolve()


def latest_v43_zip_path(repo_root: Path = REPO_ROOT) -> Path:
    candidates = sorted(
        repo_root.glob("output/supermix_omni_collective_v43_frontier_*.zip"),
        key=lambda item: (item.name, item.stat().st_mtime),
        reverse=True,
    )
    if not candidates:
        raise FileNotFoundError("No omni_collective_v43 frontier zip found under output/.")
    return candidates[0].resolve()
...
def _extract_benchmark_context(repo_root: Path) -> Dict[str, Any]:
    ...
    return {
        ...
        "v42_benchmax": _shape("v42_benchmax"),
        "omni_collective_v43": _shape("omni_collective_v43"),
        "best_overall": (
            {
...
def _load_v42_manifest(path: Path = V42_MANIFEST_PATH) -> Dict[str, Any]:
    if not path.exists():
        return {}
    return json.loads(path.read_text(encoding="utf-8"))


def build_v44_blueprint(summary: Dict[str, Any], summary_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    v42_manifest = _load_v42_manifest()
    v42_exact = _safe_float((current_benchmark.get("v42_benchmax") or {}).get("overall_exact"))
    v43_exact = _safe_float((current_benchmark.get("omni_collective_v43") or {}).get("overall_exact"))
    benchmark_gap = round(max(0.0, v42_exact - v43_exact), 4)
    ...
    return {
        ...
        "observed_gaps": [
            {
                ...
                "current": v43_exact,
                "target": max(0.25, round(v43_exact + 0.08, 4)),
                "why": "v44 needs to recover the benchmark-specialist edge from v42_benchmax while keeping v43's broader multimodal utility.",
            },
            {
                "name": "response_quality",
                "current": response_accuracy,
                "target": max(0.16, round(response_accuracy + 0.05, 4)),
                "why": "v43 improved presentation but still underperforms on response selection and final answer quality.",
            },
            {
                "name": "agentic_coding_and_verification",
                "current": domain_accuracy,
                "target": max(0.73, round(domain_accuracy + 0.04, 4)),
                "why": "v44 should route to better verifier-first coding behavior and retain stronger repair traces from Qwen3-Coder-Next style supervision.",
            },
            {
                "name": "multimodal_grounding",
                "current": vision_accuracy,
                "target": max(0.48, round(vision_accuracy + 0.08, 4)),
                "why": "Gemma 4 and Qwen3-Omni expose better grounded multimodal behaviors than the current v43 prep mix.",
            },
            {
                "name": "diversity_and_global_coverage",
                "current": intent_accuracy,
                "target": max(0.64, round(intent_accuracy + 0.05, 4)),
                "why": "Qwen3.5 explicitly broadens long-context, multilingual, and tool-use coverage that the current v43 dataset underrepresents.",
            },
        ],
        "teacher_strategy": {
            "primary_teachers": [
            ...
            "teacher_mixture_rules": [
                ...
                "Keep benchmark replay anchored to v42_benchmax plus local verifier rows.",
        ...
        "architecture": {
            ...
            "growth_vs_v43": target_parameter_count - parameter_count,
            ...
            "novel_v44_ideas": [
                "Benchmark bridge curriculum: replay v42-style hard clusters directly inside the broader v43 omni scaffold.",
        ...
        "data_program": [
            {
                "name": "v42_benchmark_bridge",
                "target_rows": 4200,
                "source_style": "v42 hard-example replay, failure clusters, and verifier-guided benchmark repair",
                "goal": "Close the measured v42-v43 benchmark gap without collapsing the omni model into a narrow benchmark specialist.",
            },
            {
            ...
            },
            {
                ...
                "goal": "Make coding behavior more deliberate and more verifiable than v43's mostly answer-level repair mix.",
            },
            {
            ...
            },
            {
                ...
                "goal": "Teach v44 to repair weak drafts with targeted feedback rather than only rerolling responses.",
            },
            {
            ...
            {
                "name": "specialist_dense_topups_v44",
                "target_rows": 2400,
        ...
        "training_program": [
            ...
            },
            {
                ...
                "recipe": "Mix v43 carryover rows with v42 hard examples and the new Gemma/Qwen role-specific teacher slices.",
            ...
            },
            {
                ...
                "recipe": "Distill good partial traces and explicit early-stop examples so v44 learns when enough reasoning is enough.",
        ...
        "success_gates": {
            ...
            "common_benchmark_exact_min": max(0.245, round(v42_exact + 0.002, 4)),
            "must_beat": ["omni_collective_v43"],
            "stretch_goal": "beat_v42_benchmax_on_common_benchmark_exact_while_preserving_omni_utility",
        },
        "promotion_protocol": {
            "required_preview_benchmarks": 4,
            "preview_selection_rule": "Promote only checkpoints that beat v43 overall and stay within multimodal regression limits while closing the v42 benchmark gap.",
            "must_not_regress": [
                "stage2 vision_accuracy vs v43 by more than 0.02",
                "stage2 domain_accuracy vs v43 by more than 0.02",
                "coding repair pack vs v43 by more than 0.01",
            ],
            "hard_gate": "No promotion if common benchmark exact remains below v42_benchmax.",
        },
        "recent_method_references": RECENT_METHOD_REFERENCES_V44,
        "v42_manifest_snapshot": {
            "objective": str(v42_manifest.get("objective") or ""),
            "innovation_flags": dict(v42_manifest.get("innovation_flags") or {}),
            "novel_features": list(v42_manifest.get("novel_features") or []),
        },
        "current_benchmark_context": current_benchmark,
        "implementation_checklist": [
            "Freeze the latest v43 frontier summary and v42 benchmark summary as the v44 design inputs.",
            "Add new v44 dataset builders for benchmark bridge, teacher-role routing, verifier repair, TurboQuant-style budget rows, and diversity expansion.",
            "Keep the v43 data pipeline as the stable base, then add v44 row groups on top instead of rewriting the whole training loop.",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Prepare a machine-readable blueprint for omni_collective_v44.")
    parser.add_argument("--summary", default="", help="Optional path to a v43 frontier summary JSON. Defaults to the latest local v43 summary.")
    parser.add_argument("--output", default=str(DEFAULT_OUTPUT_PATH), help="Where to write the v44 blueprint JSON.")
