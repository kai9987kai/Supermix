...
$logDir = Join-Path $repoRoot "output\omni_collective_v46_hybrid_evo_plus1h_train"
New-Item -ItemType Directory -Force -Path $logDir | Out-Null

$timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
$outLog = Join-Path $logDir "omni_collective_v46_hybrid_evo_plus1h_${timestamp}.out.log"
$errLog = Join-Path $logDir "omni_collective_v46_hybrid_evo_plus1h_${timestamp}.err.log"
$pidPath = Join-Path $logDir "omni_collective_v46_hybrid_evo_plus1h.pid"
$workerPidPath = Join-Path $logDir "omni_collective_v46_hybrid_evo_plus1h.worker.pid"
...
# This schedule targets roughly a one-hour follow-on evolution pass on the current CPU setup.
$stage1Epochs = 1
$stage2Epochs = 1
...
$pythonArgs = @(
    ...
    ('"{0}"' -f $latestV8Summary)
    "--output_root"
    "output\omni_collective_v46_hybrid_evo_plus1h_prep"
    "--output_dir"
    "output\omni_collective_v46_hybrid_evo_plus1h_train"
    "--family_name"
    "omni_collective_v46_hybrid_evo_plus1h"
    "--artifact_prefix"
    "supermix_omni_collective_v46_hybrid_evo_plus1h"
    "--seed"
    "811"
    "--benchmark_limit"
    "72"
