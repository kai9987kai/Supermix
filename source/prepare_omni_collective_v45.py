...
try:
    from prepare_omni_collective_v44 import latest_common_benchmark_summary_path, load_summary
except ImportError:  # pragma: no cover
    from .prepare_omni_collective_v44 import latest_common_benchmark_summary_path, load_summary


REPO_ROOT = Path(__file__).resolve().parent.parent
DEFAULT_OUTPUT_PATH = REPO_ROOT / "output" / "omni_collective_v45_blueprint.json"
V42_MANIFEST_PATH = Path(__file__).resolve().parent / "v42_benchmax_manifest.json"


RECENT_METHOD_REFERENCES_V45: List[Dict[str, str]] = [
    {
    ...
    },
    {
        ...
        "why": "Distill useful partial traces so v45 can stop earlier on easy problems without losing answer quality.",
    },
    {
    ...
    },
    {
        ...
        "why": "Use Qwen 3.5 for long-context, tool-use, and reasoning traces, especially where v44 is weak against v42_benchmax.",
    },
    {
...
def latest_v44_summary_path(repo_root: Path = REPO_ROOT) -> Path:
    candidates = sorted(
        repo_root.glob("output/supermix_omni_collective_v44_frontier_*/omni_collective_v44_frontier_summary.json"),
        key=lambda item: (item.parent.name, item.stat().st_mtime),
        reverse=True,
    )
    if not candidates:
        raise FileNotFoundError("No omni_collective_v44 frontier summary found under output/.")
    return candidates[0].resolve()


def latest_v44_zip_path(repo_root: Path = REPO_ROOT) -> Path:
    candidates = sorted(
        repo_root.glob("output/supermix_omni_collective_v44_frontier_*.zip"),
        key=lambda item: (item.name, item.stat().st_mtime),
        reverse=True,
    )
    if not candidates:
        raise FileNotFoundError("No omni_collective_v44 frontier zip found under output/.")
    return candidates[0].resolve()
...
def _extract_benchmark_context(repo_root: Path) -> Dict[str, Any]:
    ...
    return {
        ...
        "omni_collective_v44": _shape("omni_collective_v44"),
        "best_overall": (
            {
...
def build_v45_blueprint(summary: Dict[str, Any], summary_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    v44_exact = _safe_float((current_benchmark.get("omni_collective_v44") or {}).get("overall_exact"))
    benchmark_gap = round(max(0.0, v42_exact - v44_exact), 4)
    ...
    return {
        ...
        "baseline_findings": {
            "common_benchmark_exact": {
                ...
                "omni_collective_v44": v44_exact,
                "gap_to_close": benchmark_gap,
            },
            "v44_strengths_to_keep": [
        ...
        "observed_gaps": [
            {
                ...
                "current": v44_exact,
                "target": max(0.25, round(v44_exact + 0.08, 4)),
                "why": "v45 needs to recover the benchmark-specialist edge from v42_benchmax while keeping v44's broader multimodal utility.",
            },
            {
                "name": "response_quality",
                "current": response_accuracy,
                "target": max(0.16, round(response_accuracy + 0.05, 4)),
                "why": "v44 improved presentation but still underperforms on response selection and final answer quality.",
            },
            {
                "name": "agentic_coding_and_verification",
                "current": domain_accuracy,
                "target": max(0.73, round(domain_accuracy + 0.04, 4)),
                "why": "v45 should route to better verifier-first coding behavior and retain stronger repair traces from Qwen3-Coder-Next style supervision.",
            },
            {
                "name": "multimodal_grounding",
                "current": vision_accuracy,
                "target": max(0.48, round(vision_accuracy + 0.08, 4)),
                "why": "Gemma 4 and Qwen3-Omni expose better grounded multimodal behaviors than the current v44 prep mix.",
            },
            {
                "name": "diversity_and_global_coverage",
                "current": intent_accuracy,
                "target": max(0.64, round(intent_accuracy + 0.05, 4)),
                "why": "Qwen3.5 explicitly broadens long-context, multilingual, and tool-use coverage that the current v44 dataset underrepresents.",
            },
        ],
        "teacher_strategy": {
            "primary_teachers": [
        ...
        "architecture": {
            ...
            "growth_vs_v44": target_parameter_count - parameter_count,
            ...
            "novel_v45_ideas": [
                "Benchmark bridge curriculum: replay v42-style hard clusters directly inside the broader v44 omni scaffold.",
        ...
        "data_program": [
            {
                ...
                "goal": "Close the measured v42-v44 benchmark gap without collapsing the omni model into a narrow benchmark specialist.",
            },
            {
            ...
            },
            {
                ...
                "goal": "Make coding behavior more deliberate and more verifiable than v44's mostly answer-level repair mix.",
            },
            {
            ...
            },
            {
                ...
                "goal": "Teach v45 to repair weak drafts with targeted feedback rather than only rerolling responses.",
            },
            {
            ...
            {
                "name": "specialist_dense_topups_v45",
                "target_rows": 2400,
        ...
        "training_program": [
            ...
            },
            {
                ...
                "recipe": "Mix v44 carryover rows with v42 hard examples and the new Gemma/Qwen role-specific teacher slices.",
            ...
            },
            {
                ...
                "recipe": "Distill good partial traces and explicit early-stop examples so v45 learns when enough reasoning is enough.",
        ...
        "success_gates": {
            ...
            "must_beat": ["omni_collective_v44"],
            "stretch_goal": "beat_v42_benchmax_on_common_benchmark_exact_while_preserving_omni_utility",
        },
        "promotion_protocol": {
            "required_preview_benchmarks": 4,
            "preview_selection_rule": "Promote only checkpoints that beat v44 overall and stay within multimodal regression limits while closing the v42 benchmark gap.",
            "must_not_regress": [
                "stage2 vision_accuracy vs v44 by more than 0.02",
                "stage2 domain_accuracy vs v44 by more than 0.02",
                "coding repair pack vs v44 by more than 0.01",
            ],
            "hard_gate": "No promotion if common benchmark exact remains below v42_benchmax.",
        },
        "recent_method_references": RECENT_METHOD_REFERENCES_V45,
        ...
        "implementation_checklist": [
            "Freeze the latest v44 frontier summary and v42 benchmark summary as the v45 design inputs.",
            "Add new v45 dataset builders for benchmark bridge, teacher-role routing, verifier repair, TurboQuant-style budget rows, and diversity expansion.",
            "Keep the v44 data pipeline as the stable base, then add v45 row groups on top instead of rewriting the whole training loop.",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Prepare a machine-readable blueprint for omni_collective_v45.")
    parser.add_argument("--summary", default="", help="Optional path to a v44 frontier summary JSON. Defaults to the latest local v44 summary.")
    parser.add_argument("--output", default=str(DEFAULT_OUTPUT_PATH), help="Where to write the v45 blueprint JSON.")
