...
import train_omni_collective_v7
...
class TeeStream(io.TextIOBase):
    ...
    def write(self, s: str) -> int:  # pragma: no cover - thin I/O wrapper
        text = str(s)
    ...
    def flush(self) -> None:  # pragma: no cover - thin I/O wrapper
        for stream in self._streams:
            stream.flush()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Background launcher for omni_collective_v7 with in-process log teeing.")
...
def main() -> None:
    ...
    try:
        ...
        sys.argv = ["train_omni_collective_v7.py"] + train_args
        train_omni_collective_v7.main()
    except Exception:
        traceback.print_exc()
        raise
    finally:
