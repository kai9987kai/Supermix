...
$runName = "omni_collective_v46_meta_evo420_plus2h"
...
$latestBaseDir = Join-Path $repoRoot "output\omni_collective_v46_meta_evo360_train"
$latestBaseSummary = Get-LatestSingleFile -Path $latestBaseDir -Filter "omni_collective_v46_meta_evo360_frontier_summary.json"
$latestBaseZip = Get-LatestSingleFile -Path $latestBaseDir -Filter "supermix_omni_collective_v46_meta_evo360_frontier_*.zip"
$latestV8Summary = Get-LatestSingleFile -Path (Join-Path $repoRoot "output") -Filter "omni_collective_v8_frontier_summary.json"

# Keep this run checkpoint-first. The watchdog can be disabled when enough
# disk space is available and we want the checkpointed run to finish naturally.
...
$pythonArgs = @(
    ...
    ('"{0}"' -f $latestV8Summary)
    "--output_root"
    "output\omni_collective_v46_meta_evo420_plus2h_prep"
    "--output_dir"
    "output\omni_collective_v46_meta_evo420_plus2h_train"
    "--family_name"
    "omni_collective_v46_meta_evo420_plus2h"
    "--artifact_prefix"
    "supermix_omni_collective_v46_meta_evo420_plus2h"
    "--seed"
    "1807"
    "--benchmark_limit"
    "96"
...
[pscustomobject]@{
    ...
    benchmark_failure_replay_limit = 240
