...
from model_native_image_lite_v37 import ChampionNetUltraExpertNativeImageLite
...
def _resolve_feature_mode(teacher_meta: Path | None, explicit_mode: str) -> str:
    if explicit_mode:
        return explicit_mode
    if teacher_meta is not None and teacher_meta.exists():
        try:
            meta = json.loads(teacher_meta.read_text(encoding="utf-8"))
        except Exception:
            meta = {}
...
def _load_matching_weights(model: ChampionNetUltraExpertNativeImageExtraLite, weights_path: Path) -> Dict[str, int]:
    source = safe_load_state_dict(str(weights_path))
    ...
    return {"matched_keys": len(matched), "skipped_keys": len(skipped)}


def _load_teacher_model(weights_path: Path, device: torch.device) -> ChampionNetUltraExpertNativeImageLite:
    model = ChampionNetUltraExpertNativeImageLite(image_size=64).to(device)
...
def _set_trainable_layers(model: ChampionNetUltraExpertNativeImageExtraLite, *, freeze_text_backbone: bool, unfreeze_text_layers: int) -> None:
    for param in model.parameters():
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Distill an extra-lite native-image model from v37 lite.")
    parser.add_argument("--pairs", required=True)
    parser.add_argument("--teacher_weights", required=True)
    parser.add_argument("--teacher_meta", default="")
    parser.add_argument("--student_init_weights", required=True)
    ...
    parser.add_argument("--unfreeze_text_layers", type=int, default=2)
    parser.add_argument("--gt_image_weight", type=float, default=1.0)
    parser.add_argument("--teacher_image_weight", type=float, default=0.60)
    parser.add_argument("--teacher_text_weight", type=float, default=0.10)
    ...
    student_init_weights = Path(args.student_init_weights)
    ...
    feature_mode = _resolve_feature_mode(teacher_meta, str(args.feature_mode or ""))
    ...
    student = ChampionNetUltraExpertNativeImageExtraLite(image_size=int(args.image_size)).to(device)
    init_report = _load_matching_weights(student, student_init_weights)
    ...
    if not trainable_params:
        raise ValueError("No trainable parameters selected for v38 extra-lite native image distillation.")
    ...
    meta_payload = {
        "variant": "v38_native_image_extra_lite_single_checkpoint",
    ...
    if sample_output is not None:
        ...
        print(f"[v38-xlite-distill] sample={sample_output}")

    print(f"[v38-xlite-distill] weights={output_weights}")
    print(f"[v38-xlite-distill] meta={output_meta}")
    print(f"[v38-xlite-distill] summary={summary_out}")
