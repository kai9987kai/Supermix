...
def infer_style_mode(
    ...
    # never activates.
    #
    # Checked after the current turn's own explicit concise cue and before the
    # content-based inferences below, so a fresh instruction always wins over a
    # standing one while an inferred topic mode does not override what the user
    # actually asked for.
    if isinstance(conversation_state, Mapping) and not DETAIL_REQUEST_RE.search(query):
        ...
        # having no memory at all. A fresh explicit request always outranks a
        # standing preference.
