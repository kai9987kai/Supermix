...
$runName = "omni_collective_v46_common_v6_userbench_evo2h"
...
$newUserBenchDetails = Get-ChildItem -Path (Join-Path $repoRoot "output") -Recurse -File -Filter "benchmark_all_models_common_details.jsonl" -ErrorAction SilentlyContinue |
    Where-Object { $_.FullName -match "userbench|common_v6|common_v5_20suite|common_v4_hard|cachefloor" } |
    Sort-Object LastWriteTime -Descending |
    Select-Object -First 1
...
$pythonArgs = @(
    ...
    ('"{0}"' -f $latestV8Summary),
    "--output_root",
    "output\omni_collective_v46_common_v6_userbench_evo2h_prep",
    "--output_dir",
    "output\omni_collective_v46_common_v6_userbench_evo2h_train",
    "--family_name",
    "omni_collective_v46_common_v6_userbench_evo2h",
    "--artifact_prefix",
    "supermix_omni_collective_v46_common_v6_userbench_evo2h",
    "--seed",
    "2114",
    "--benchmark_limit",
    "340",
...
[pscustomobject]@{
    ...
    target_hours = if ($NoWatchdog) { $null } else { 2 }
    out_log = $outLog
    err_log = $errLog
    ...
    newest_userbench_details = if ($newUserBenchDetails) { $newUserBenchDetails.FullName } else { "" }
    benchmark_suite_version = "common_v6_25suite_user_understanding"
    strategy = "promoted v46 warm-start + 20-suite retention + new user-understanding suite seed replay + chat relevance drift repair + weak-suite oversampling"
    base_branch = [string]$championManifest.champion_family
    base_20suite_score = [double]$championManifest.common_benchmark_score
    baseline_25suite_score = 0.872
    new_userbench_baseline = "new5 overall 0.360: user_intent 0.800, context_tracking 0.400, instruction_following 0.200, ambiguity_resolution 0.200, chat_relevance 0.200"
    target = "raise the new user-understanding suites while preserving the existing 20-suite exact score"
