...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run one v7 teacher model in isolation and persist resumable state.")
...
def main() -> None:
    ...
    if not str(os.environ.get("SUPERMIX_QWEN_BASE_MODEL_DIR") or "").strip():
        try:
            os.environ["SUPERMIX_QWEN_BASE_MODEL_DIR"] = str(train_v7._resolve_local_qwen_base_model())
        except Exception:
            pass

    sample = train_v7._load_teacher_sample_v7(sample_path)
    if not sample:
    ...
    extraction_root = (repo_root / "output" / "omni_v7_teacher_cache").resolve()
    generated_dir = (repo_root / "output" / "omni_v7_teacher_generated").resolve()
    ...
    def save_state(status: str) -> None:
        ...
        train_v7._write_json_atomic(state_path, payload)
    ...
    try:
        for row_index, row in enumerate(sample, start=1):
            if row_index in cached_rows or row_index in empty_row_indices:
                continue
            session_id = f"v7distill::{teacher_key}::{row_index}"
            try:
                result = backend.chat(
                    session_id=session_id,
                    prompt=row.prompt,
                    settings={
                        "style_mode": train_v7._model_style_for_row_v7(row),
            ...
            if not candidate:
                ...
                score = float(train_v7._score_candidate(row.response_text, candidate)["composite"])
