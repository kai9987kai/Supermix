...
$runName = "omni_collective_v46_champion_guard_evo4h"
...
$championDir = Join-Path $repoRoot "output\omni_collective_v46_meta_evo420_plus2h_train"
$championSummary = Get-LatestSingleFile -Path $championDir -Filter "omni_collective_v46_meta_evo420_plus2h_frontier_summary.json"
$championZip = Get-LatestSingleFile -Path $championDir -Filter "supermix_omni_collective_v46_meta_evo420_plus2h_frontier_*.zip"
$latestV8Summary = Get-LatestSingleFile -Path (Join-Path $repoRoot "output") -Filter "omni_collective_v8_frontier_summary.json"

# The previous continuation collapsed because the warm-start classifier was
# evaluated against a shifted response bank. This run uses the fixed append-only
# label-space path and keeps the 0.5167 common benchmark champion as the base.
$env:OMNI_V46_MIN_FREE_GB = "1.5"
$stage1Epochs = 3
$stage2Epochs = 2
...
$pythonArgs = @(
    ...
    ('"{0}"' -f $latestV8Summary)
    "--output_root"
    "output\omni_collective_v46_champion_guard_evo4h_prep"
    "--output_dir"
    "output\omni_collective_v46_champion_guard_evo4h_train"
    "--family_name"
    "omni_collective_v46_champion_guard_evo4h"
    "--artifact_prefix"
    "supermix_omni_collective_v46_champion_guard_evo4h"
    "--seed"
    "2026"
    "--benchmark_limit"
    "128"
...
[pscustomobject]@{
    ...
    label_space_policy = "base append-only"
    benchmark_failure_replay_limit = 360
    cognitive_evolution_limit = 420
    research_evolution_limit = 260
    agentic_evolution_limit = 224
