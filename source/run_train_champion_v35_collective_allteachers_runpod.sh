...
OUTPUT_DIR="${OUTPUT_DIR:-${PERSIST_ROOT}/artifacts/champion_v35_collective_allteachers_20260326}"
...
START_WEIGHTS="${START_WEIGHTS:-${V34_STAGE2_WEIGHTS}}"
START_META="${START_META:-${V34_STAGE2_META}}"
BASELINE_WEIGHTS="${BASELINE_WEIGHTS:-${V34_STAGE2_WEIGHTS}}"
BASELINE_META="${BASELINE_META:-${V34_STAGE2_META}}"

DISTILL_DATA="${DISTILL_DATA:-${OUTPUT_DIR}/v35_collective_allteachers_distill_train.jsonl}"
STAGE2_TRAIN_DATA="${STAGE2_TRAIN_DATA:-${OUTPUT_DIR}/v35_collective_allteachers_stage2_train_mix.jsonl}"
STAGE2_EVAL_DATA="${STAGE2_EVAL_DATA:-${OUTPUT_DIR}/v35_collective_allteachers_stage2_eval_mix.jsonl}"
STAGE3_POLISH_DATA="${STAGE3_POLISH_DATA:-${OUTPUT_DIR}/v35_collective_allteachers_stage3_polish.jsonl}"
DISTILL_SUMMARY="${DISTILL_SUMMARY:-${OUTPUT_DIR}/v35_collective_allteachers_dataset_summary.json}"
DISTILL_AUDIT="${DISTILL_AUDIT:-${OUTPUT_DIR}/v35_collective_allteachers_dataset_audit.jsonl}"

STAGE1_OUT="${STAGE1_OUT:-${OUTPUT_DIR}/champion_model_chat_v35_collective_allteachers_stage1.pth}"
STAGE1_META="${STAGE1_META:-${OUTPUT_DIR}/chat_model_meta_v35_collective_allteachers_stage1.json}"
STAGE2_OUT="${STAGE2_OUT:-${OUTPUT_DIR}/champion_model_chat_v35_collective_allteachers_stage2.pth}"
STAGE2_META="${STAGE2_META:-${OUTPUT_DIR}/chat_model_meta_v35_collective_allteachers_stage2.json}"
STAGE3_OUT="${STAGE3_OUT:-${OUTPUT_DIR}/champion_model_chat_v35_collective_allteachers_stage3.pth}"
STAGE3_META="${STAGE3_META:-${OUTPUT_DIR}/chat_model_meta_v35_collective_allteachers_stage3.json}"
CHOSEN_JSON="${CHOSEN_JSON:-${OUTPUT_DIR}/v35_collective_allteachers_chosen_checkpoint.json}"

BUILD_LOG="${BUILD_LOG:-${LOG_DIR}/build_v35_collective_allteachers.out.log}"
STAGE1_LOG="${STAGE1_LOG:-${LOG_DIR}/train_champion_v35_collective_allteachers_stage1.out.log}"
STAGE2_LOG="${STAGE2_LOG:-${LOG_DIR}/train_champion_v35_collective_allteachers_stage2.out.log}"
STAGE3_LOG="${STAGE3_LOG:-${LOG_DIR}/train_champion_v35_collective_allteachers_stage3.out.log}"
BENCH_STAGE1_LOG="${BENCH_STAGE1_LOG:-${LOG_DIR}/benchmark_champion_v35_stage1_vs_v34.out.log}"
BENCH_STAGE2_LOG="${BENCH_STAGE2_LOG:-${LOG_DIR}/benchmark_champion_v35_stage2_vs_stage1.out.log}"
BENCH_STAGE3_LOG="${BENCH_STAGE3_LOG:-${LOG_DIR}/benchmark_champion_v35_stage3_vs_stage2.out.log}"
BENCH_CHOSEN_LOG="${BENCH_CHOSEN_LOG:-${LOG_DIR}/benchmark_champion_v35_chosen_vs_v34.out.log}"
SUMMARY_JSON="${SUMMARY_JSON:-${OUTPUT_DIR}/v35_collective_allteachers_training_summary.json}"
FINAL_ZIP="${FINAL_ZIP:-${PERSIST_ROOT}/champion_v35_collective_allteachers_full_model_20260326.zip}"
BUNDLE_ZIP="${BUNDLE_ZIP:-${PERSIST_ROOT}/champion_v35_collective_allteachers_full_bundle_20260326.zip}"
...
for required in \
  ...
  "${BASELINE_META}"; do
  if [[ ! -e "${required}" ]]; then
    echo "[v35] missing required path: ${required}" >&2
    exit 1
  fi
done
...
EXTRA_QWEN_ARGS+=(--extra_qwen_teacher "qwen_v29=${QWEN_V29_ADAPTER}")
EXTRA_QWEN_ARGS+=(--extra_qwen_teacher "qwen_v30=${QWEN_V30_ADAPTER}")
...
BUILD_CMD=(
  python3
  source/build_v35_allteachers_dataset.py
  --prompt_data
  "${PROMPT_DATA_1}"
  "${PROMPT_DATA_2}"
  "${PROMPT_DATA_3}"
  ...
  --skip_champion_v28
  --sample_size 1280
  --stage2_reference_cap 5600
  --eval_fraction 0.16
  --eval_cap 448
  --paper_eval_fraction 0.25
  --paper_eval_cap 40
  --polish_limit 576
  --self_repair_limit 576
  --seed 91
  --device cuda
  --max_new_tokens 120
  --log_every 32
  --min_teacher_score 0.17
  "${EXTRA_QWEN_ARGS[@]}"
  "${EXTRA_CHAMPION_ARGS[@]}"
)

printf '[v35] build command:\n %q' "${BUILD_CMD[@]}"
printf '\n'
...
STAGE1_CMD=(
  ...
  --output "${STAGE1_OUT}"
  --meta "${STAGE1_META}"
  --model_size frontier_collective_expert
...
printf '[v35] stage1 train command:\n %q' "${STAGE1_CMD[@]}"
printf '\n'
...
printf '[v35] stage1 benchmark command:\n %q' "${BENCH_STAGE1_CMD[@]}"
printf '\n'
...
printf '[v35] stage2 train command:\n %q' "${STAGE2_CMD[@]}"
printf '\n'
...
printf '[v35] stage2 benchmark command:\n %q' "${BENCH_STAGE2_CMD[@]}"
printf '\n'
...
printf '[v35] stage3 train command:\n %q' "${STAGE3_CMD[@]}"
printf '\n'
...
printf '[v35] stage3 benchmark command:\n %q' "${BENCH_STAGE3_CMD[@]}"
printf '\n'
...
echo "[v35] chosen final checkpoint: ${CHOSEN_STAGE}"
echo "[v35] chosen final weights: ${CHOSEN_WEIGHTS}"
...
printf '[v35] chosen benchmark command:\n %q' "${BENCH_CHOSEN_CMD[@]}"
printf '\n'
...
summary = {
    ...
    "weights_bytes": {
        "baseline_v34": baseline_weights_path.stat().st_size,
        "start_v34_stage2": start_weights_path.stat().st_size,
    ...
    "note": (
        "The v35 collective all-teachers run distills every loadable trained text checkpoint on the pod into a larger frontier_collective_expert student, "
        "then keeps the strongest checkpoint automatically."
    ),
}
...
python3 - \
  ...
  "${REPO_ROOT}/source/build_v35_allteachers_dataset.py" \
  ...
  "${REPO_ROOT}/source/run_train_champion_v35_collective_allteachers_runpod.sh" <<'PY'
import sys
import zipfile
from pathlib import Path
...
echo "[v35] final model zip: ${FINAL_ZIP}"
echo "[v35] bundle zip: ${BUNDLE_ZIP}"
echo "[v35] summary: ${SUMMARY_JSON}"
