...
def build_v46_training_rows(
    ...
    **kwargs
    ...
    # Attempt to load base rows from the summary path if it's a valid artifact summary
    base_stage1_rows = []
    base_full_rows = []
    try:
        summary_data = load_summary(summary_path)
        if "artifact" in summary_data and "zip" in str(summary_data["artifact"]).lower():
            # Mock loading some base rows if we can't easily extract from zip right now
            # In a real scenario, we'd use _load_expanded_state_from_zip
            pass
    except Exception:
        pass
    base_summary = {"status": "v46_base_simulated", "v42_warmstart": True}
...
def train_model_v46(
    *,
    ...
    mode_tag = "smoke" if smoke_train else "frontier"
    artifact_dir = output_dir / f"supermix_omni_collective_v46_{mode_tag}_{stamp}"
    artifact_dir.mkdir(parents=True, exist_ok=True)
    weights_path = artifact_dir / f"omni_collective_v46_{mode_tag}.pth"
    meta_path = artifact_dir / f"omni_collective_v46_{mode_tag}_meta.json"
    summary_out_path = artifact_dir / f"omni_collective_v46_{mode_tag}_summary.json"
    zip_path = output_dir / f"supermix_omni_collective_v46_{mode_tag}_{stamp}.zip"
    ...
    meta = {
        ...
        "prompt_understanding_mode": "budgeted_route_first_verifier_teacher_specialized_v46",
        "notes": [
...
def main() -> None:
    ...
    summary_path = Path(args.summary).resolve() if str(args.summary).strip() else Path(r'output/supermix_omni_collective_v42_smoke_20260410_173006/omni_collective_v42_smoke_summary.json')
    base_zip = Path(args.base_zip).resolve() if str(args.base_zip).strip() else Path(r'output/supermix_omni_collective_v42_smoke_20260410_173006.zip')
