...
OmniCollectiveNetV6 = OmniCollectiveNetV4


def _prompt_variants_v6(prompt: str, draft_answer: str = "", *, response_confidence: float = 0.0) -> List[str]:
    ...
    variants = [
        ...
        (
            ...
            "Preserve any explicit formatting constraints, keep the answer on-topic, and avoid unrelated stock phrasing."
        ),
        (
            "Ground the answer in what is actually supported by the request and local knowledge.\n"
            f"User request: {prompt_text}\n"
            "If you are uncertain, prefer a narrower answer over an invented one."
        ),
        (
            "Re-read the prompt for hidden constraints.\n"
            f"Request: {prompt_text}\n"
            "Decide the domain, desired output style, and any forbidden drift before answering."
        ),
    ]
    if draft_answer:
        variants.append(
            (
                "Review this candidate answer for unsupported claims, hallucinated specifics, and off-topic drift.\n"
                f"Request: {prompt_text}\n"
                f"Candidate answer: {draft_answer}\n"
                "Keep correct parts, remove unsupported claims, and tighten the final answer."
            )
        )
    if response_confidence < 0.46:
        variants.append(
            (
                "Confidence is low. Produce the most grounded answer you can.\n"
                f"Request: {prompt_text}\n"
                "Prefer precise uncertainty and bounded claims over speculative detail."
            )
        )
    if focus == "coding":
        variants.append(
            (
                ...
                f"Request: {prompt_text}\n"
                "Prefer exact syntax, concrete debugging steps, reproducible reasoning, and technically correct explanations."
            )
        )
    elif focus == "openscad":
        variants.append(
            (
                ...
                f"Request: {prompt_text}\n"
                "Prefer valid OpenSCAD primitives, transforms, boolean ops, modules, and parameterized examples."
            )
        )
        ...
        variants.append(
            (
                ...
                f"Request: {prompt_text}\n"
                "Resolve the user's true goal, hidden constraints, and safest direct answer before responding."
            )
        )
    elif focus == "general":
        variants.append(
    ...
    if any(token in prompt_text.lower() for token in ("math", "equation", "integrate", "differentiate", "solve", "factor")):
        variants.append(
            (
                "This request may involve math.\n"
                f"Request: {prompt_text}\n"
                "Prefer exact symbolic reasoning or a clean, step-safe final statement over loose intuition."
            )
        )
    deduped: List[str] = []
    seen = set()
...
class OmniPredictionV6:
    ...
    reasoning_mode: str = "grounded_consensus_deliberation"


class OmniCollectiveEngineV6:
    def __init__(self, *, weights_path: Path, meta_path: Path, device: Optional[torch.device] = None) -> None:
        ...
        self.max_words = int(self.meta.get("max_words") or 80)
        self.deliberation_passes = int(self.meta.get("deliberation_passes") or 6)
        self.minimum_passes = int(self.meta.get("minimum_passes") or 3)
        self.grounding_threshold = float(self.meta.get("grounding_threshold") or 0.46)
        self.device = device or torch.device("cuda" if torch.cuda.is_available() else "cpu")

        self.model = OmniCollectiveNetV6(
            ...
            base_embed_dim=int(self.meta.get("embed_dim") or 104),
            text_hidden=int(self.meta.get("text_hidden") or 208),
            image_channels=int(self.meta.get("image_channels") or 44),
            word_buckets=int(self.meta.get("word_buckets") or 16384),
            word_embed_dim=int(self.meta.get("word_embed_dim") or 96),
            deep_text_channels=int(self.meta.get("deep_text_channels") or 272),
            deep_image_channels=int(self.meta.get("deep_image_channels") or 96),
            fusion_hidden=int(self.meta.get("fusion_hidden") or 768),
            memory_slots=int(self.meta.get("memory_slots") or 16),
            depth_steps=int(self.meta.get("depth_steps") or 7),
            expert_count=int(self.meta.get("expert_count") or 6),
            expert_hidden=int(self.meta.get("expert_hidden") or 1152),
    ...
    def predict(self, prompt: str, image_path: Optional[str] = None) -> OmniPredictionV6:
        ...
        response_conf, _domain_conf = self._confidence_from_outputs(raw_outputs)
        ...
        variants = _prompt_variants_v6(prompt_text, draft_answer, response_confidence=response_conf)

        weighted_outputs: List[tuple[float, Dict[str, torch.Tensor]]] = []
        combined = raw_outputs
        for index, variant in enumerate(variants, start=1):
            weight = 1.18 if index == 1 else 1.0
            if "unsupported claims" in variant:
                weight = 1.08
            ...
            if index >= self.minimum_passes and response_conf >= self.grounding_threshold and domain_conf >= 0.40:
                break
    ...
    def answer(self, prompt: str, image_path: Optional[str] = None) -> str:
        ...
        if prediction.response_confidence < self.grounding_threshold and prediction.domain in {"knowledge", "general", "planning", "language"}:
            if prediction.response_text:
