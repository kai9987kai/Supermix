"""v35 larger frontier architecture for all-teacher collective distillation.

This variant keeps the v33/v34 frontier-compatible head keys so it can warm-start
from the current best checkpoint, then adds extra collective synthesis branches to
increase capacity for multi-teacher aggregation.
"""
...
class FrontierCollectiveExpertHead(nn.Module):
    """Larger frontier head that keeps v34-compatible parameters and adds new branches."""
    ...
    def __init__(
        ...
        # New collective aggregation paths for v35.
        self.shared_aux_up = nn.Linear(in_dim, 3072, bias=False)
        self.shared_aux_down = nn.Linear(3072, out_dim, bias=False)
        self.shared_aux_norm = nn.LayerNorm(out_dim)
        self.shared_aux_gate = nn.Linear(in_dim, 2, bias=True)

        self.reflect_cells = nn.ModuleList([ReasoningCell(in_dim, inner_dim=1024, dropout=dropout) for _ in range(2)])
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if expert_usage:
            ...
            mix_loss = (mix_entropy - 0.60).pow(2)
            self._aux_loss = balance_loss + 0.30 * halt_loss + 0.20 * mem_loss + 0.15 * mix_loss
        else:
...
class ChampionNetFrontierCollectiveExpert(nn.Module):
    """Backbone wrapper for the larger v35 collective frontier head."""
